...
% tau = (exposure t) R_p
%==========================================================================
function m1val = m1(Gm,Vr,tau,Proots)

m1val = (-Vr/2)*(f(Gm,Vr,tau,Proots(1))+f(Gm,Vr,tau,Proots(2))+...
...
fval = ((-4).*exp(1).^((1/4).*tau.*x)+(-416).*exp(1).^((1/4).*tau.* ...
  x).*Gm.*pi.^2+(-9216).*exp(1).^((1/4).*tau.*x).* ...
  Gm.^2.*pi.^4+exp(1).^((1/4).*tau.*x).*Vr.^2+(-4).*exp(1).^(( ...
  1/4).*tau.*x).*x+(-208).*exp(1).^((1/4).*tau.*x) ...
  .*Gm.*pi.^2.*x+(-1).*exp(1).^((1/4).*tau.*x).*x ...
  .^2).*((-8)+(-672).*Gm.*pi.^2+(-12544).*Gm.^2.*pi.^4+(-36864).* ...
  Gm.^3.*pi.^6+4.*Vr.^2+144.*Gm.*pi.^2.*Vr.^2+(-12).*x+(-672) ...
  .*Gm.*pi.^2.*x+(-6272).*Gm.^2.*pi.^4.*x+2.*Vr.^2.* ...
 x+(-6).*x.^2+(-168).*Gm.*pi.^2.*x.^2+(-1).*x ...
  .^3).^(-1);
