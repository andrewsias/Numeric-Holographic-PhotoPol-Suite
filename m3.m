...
% tau = (exposure t) R_p
%==========================================================================
function m3val = m3(Gm,Vr,tau,Proots)

m3val = (-Vr.^3/2)*(f(Gm,Vr,tau,Proots(1))+f(Gm,Vr,tau,Proots(2))+...
...
fval = exp(1).^((1/4).*tau.*x).*(8+672.*Gm.*pi.^2+12544.*Gm.^2.* ...
pi.^4+36864.*Gm.^3.*pi.^6+(-4).*Vr.^2+(-144).*Gm.*pi.^2.*Vr.^2+ ...
12.*x+672.*Gm.*pi.^2.*x+6272.*Gm.^2.*pi.^4.*x+( ...
  -2).*Vr.^2.*x+6.*x.^2+168.*Gm.*pi.^2.*x.^2+x.^3).^(-1);
